...
(function ($) {
    ...
    $('.lia-new-attachments-field .lia-new-attachment:eq(1)').before('<legend class="lia-fieldset-title">JSON File</legend>');
    /* Added Required Astriek to the JSON file field(MC-120) */
    $('.lia-new-attachments-field .lia-fieldset-title').append("<span class='field-required'>*</span>");
    ...
    /* Changed the accept file type to .json(MC-120) */
    $('.lia-new-attachments-field input.lia-form-type-file:eq(1)').attr('accept', '.json');
    ...
    $('.lia-button-Submit-action').bind('click', function(e) {
        ...
        if(optional.toLowerCase() != "true" || $('#microsoftmsg_report_url').val() != "") {
            /* var urlValidatorRegex = /^(http|ftp|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])?$/; */
             ...
             /*Code is modified as part of the MC-405 iTrack */
            var urlValidatorRegex = /^(http|https):(\/\/app.powerbi.com\/view\?r=)([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])?$/;
           /* End code is modified as part of the MC-405 iTrack */
        ...
        var jsonAttached = false;
        var jsonAttachedTester = /\.json/;
       $.each($('.lia-new-attachments-field .lia-new-attachment .lia-form-type-file'), function (index,  value) {
            if (imageAttachedTester.test($(value).val().toLowerCase())) {
                imageAttached = true;
            }

             if (jsonAttachedTester.test($(value).val().toLowerCase())) {
                jsonAttached = true;
        ...
        $.each($('.lia-new-attachments-field .lia-new-attachment .lia-form-type-file'), function (index, value) {
            ...
            if (jsonAttachedTester.test($(value).text().toLowerCase())) {
                jsonAttached = true;
            }
        }); 

        if (!imageAttached) {
            if ($('.lia-form-attachments-fieldset-toggle .lia-add-new-attachment:eq(0):checked').length == 0 || $('.lia-new-attachments-field .lia-new-attachment .lia-form-type-file').eq(0).val().length ==0) {
...
/* Checking JSON file attached case(MC-120) */
        if (!jsonAttached) {
            if ($('.lia-form-attachments-fieldset-toggle .lia-add-new-attachment:eq(1):checked').length == 0 || $('.lia-new-attachments-field .lia-new-attachment .lia-form-type-file').eq(1).val().length ==0) {
                ...
                if ($('.lia-form-attachments-fieldset-toggle .lia-new-attachment:eq(1) + p').length == 0) {
                    $('.lia-form-attachments-fieldset-toggle .lia-new-attachment:eq(1)').after('<p role="alert" id="describedBy_microsoftmsg_attachments" class="lia-form-error-text">Please attach a JSON file.</p>');
                }
...
/* Cheecking the type of attached file(MC-120) */
        var jsonAttachedTester = /\.json/;
        var jsonFile = $('.lia-form-attachments-fieldset-toggle .lia-form-type-file:eq(1)').val();
        if (jsonFile != "" && !jsonAttachedTester.test(jsonFile.toLowerCase())) {
            $('.lia-form-attachments-fieldset-toggle .lia-new-attachment:eq(1)').addClass('lia-form-field-error');

            if ($('.lia-form-attachments-fieldset-toggle .lia-new-attachment:eq(1) + p').length == 0) {
                $('.lia-form-attachments-fieldset-toggle .lia-new-attachment:eq(1)').after('<p role="alert" id="describedBy_microsoftmsg_attachments" class="lia-form-error-text">The Report theme file type should be \'.JSON\'.</p>');
            }

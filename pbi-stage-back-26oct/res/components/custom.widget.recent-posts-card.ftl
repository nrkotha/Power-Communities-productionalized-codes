...
<div class="header_bottom">
    <div class="col-md-6">
	  <div>
        <h2 class="community-blog">${nodeDetails.title}</h2>
	  </div>
	</div>
	<div class="col-md-6">
	   <div class="view-all">
         <a href="${nodeDetails.href}">View All</a>
	   </div>
	</div>
</div>
		    ...
		    <div class="cmBlog-Widget">
		       <div class="bg-communityBlog">
					<figure class="user-profile">  
						<img class="bitmap ${(postResult.author.login='')?then('','lia-user-avatar-message')}" title="${postResult.author.login}" src="${postResult.author.avatar.message}">
					</figure>
					<div class="user-info">
						<a href="${(postResult.author.login='')?then('JavaScript:Void(0)',postResult.author.view_href)}"><span class="username">${(postResult.author.login='')?then('Anonymous',item.author.login)}</span></a><br/>
				...
				<div class="layer-img">
					<figure>  
						...
						<span>
${(item.kudos.count??&&item.kudos.count?has_content)?then(item.kudos.count,0)}
						</span>
					</figure> 
					<figure>  
						<img class="layer" alt="comments"  src="${asset.get('/html/assets/comments.png')}">
						<span>${postResult.conversation.messages_count-1}</span>
					</figure> 
					<figure>  

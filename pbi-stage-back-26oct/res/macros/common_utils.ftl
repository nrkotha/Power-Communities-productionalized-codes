...
<#function underEventCategory>
  ...
  <#if (coreNode.id)?? && (coreNode.id == eventId || coreNode.id = "grazitti-events") >
    <#return true />
  </#if>
...
<#macro generateNavContent HomePageTitle HomePageLink>
 ...
 <a class="navbar-brand" href="${HomePageLink}">${HomePageTitle}</a>
    <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#navBarLinks_top">
    <span class="icon-bar"></span>
    <span class="icon-bar"></span>
    <span class="icon-bar"></span>                        
    </button>
  </div>
  <div class="collapse navbar-collapse main_menu" id="navBarLinks_top" >
    <ul class="nav navbar-nav tp_left_menu">  
        <#nested />
    </ul>
  </div>
</#macro>
  ...
  <li class="category">
    ...
    <#if restQuery !="">  <#-- Start (Empty Query) -->
      ...
      <#if (queryResult?size > 0)>
        ...
        <ul class="custom-navbar-boardUl menu_list">
...
<#macro moreDropdownLinks title >
  <li class="dropdown category">
      ...
      <ul class="dropdown-menu more-menu menu_list">
        <#nested />
...
<#assign num= (number / (Map[thousands].factor))?string("0.0")/>
<#assign condition=(num?number)?floor+".0"/>
<#if num=condition>
  <#assign Value = (num?number)?floor + Map[thousands].unit />
<#else>
<#assign Value = num + Map[thousands].unit />
</#if>
  <#return Value />
</#function>
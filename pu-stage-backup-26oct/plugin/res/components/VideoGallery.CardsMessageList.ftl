<#--
    ...
    Video Story Gallery: A Gallery where users can post Videos from Youtube. 
    iTrack : PBI – 1988. 
...
<#--
    ...
    Description:
        ...
        This component is created to display grid structure of all the videos posted to the board. 

    Activities:
        1. Displays grid structure of all videos posted to the board.
        2. Has the functionality to filter the videos through Labels.
        3. Has the functionality to filter the videos through custom filters Featured, Top Kudos and Recently posted.          
...
<#if sourceBoard=="oxcrx34285"> <!-- for preview only -->
    <#assign sourceBoard="VideoGallery" />
</#if>
...
<#assign viewCardText = coreNode.settings.name.get("customcontent.9_text")!""/>
<#if viewCardText =="">
	<#assign viewCardText  = {}/>
<#else>
<#attempt>
	<#assign viewCardText  = viewCardText?eval/>
<#recover>
</#attempt>
</#if>
<#attempt>
    <#assign cardText = viewCardText.galleryViewCardText />
...
<#if newMessageText==customNewMessageText>
    <#assign newMessageText="Submit a video" />
</#if>
            ...
            <#if post??>
                ...
                <div class="lia-messages-message-card">
                    <div class="lia-messages-message-card-container">
                        <a href="${view_href}">
						<#if cardText != "">
							<div class="lia-messages-message-card-image-wrapper" text="${cardText}">
						<#else>
                            <div class="lia-messages-message-card-image-wrapper" text="View Video">
						</#if>	
...
(function ($) {
	...
	<#assign videoData = coreNode.settings.name.get("customcontent.9_text")!""/>
		<#assign DeniedVideoButtonText=""/>
		<#assign DeniedVideoButtonURL=""/>
	<#attempt>
		<#if videoData?? && videoData?has_content>
			<#assign videoData  = videoData?eval/>
			<#if videoData.DeniedVideoButtonText?? && videoData.DeniedVideoButtonText?has_content>
				<#assign DeniedVideoButtonText=videoData.DeniedVideoButtonText/>
			</#if>
			<#if videoData.DeniedVideoButtonURL?? && videoData.DeniedVideoButtonURL?has_content>
				<#assign DeniedVideoButtonURL=videoData.DeniedVideoButtonURL/>
			</#if>
		</#if>
	<#recover>
	</#attempt>
	try{
		if(($('.lia-menu-bar .lia-button-primary.lia-link-disabled').length >0) && ('${DeniedVideoButtonText?url}'!='') && ('${DeniedVideoButtonURL?url}'!='')){
			$(".lia-menu-bar-buttons .lia-button-wrapper.lia-button-wrapper-primary.lia-link-disabled").removeClass("lia-link-disabled")
			var newHtml = '<a class="lia-button lia-button-primary" href="${DeniedVideoButtonURL?js_string}"><span>${DeniedVideoButtonText?js_string}</span></a></span>';

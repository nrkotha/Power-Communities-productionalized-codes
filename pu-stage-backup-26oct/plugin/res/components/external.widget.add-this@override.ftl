...
<#assign twitterTag = "%23PowerUsers"/>
<#if skin.id?contains("flow")>
<#assign twitterTag = "%23MicrosoftFlow"/>
<#elseif skin.id?contains("powerapps")>
<#assign twitterTag = "%23PowerApps"/>
<#else>

</#if>
    <a href="http://twitter.com/share?text=Check out this page: ${env.context.message.subject} ${twitterTag}&url=${env.context.message.webUi.url}" class="social-share social-twitter">

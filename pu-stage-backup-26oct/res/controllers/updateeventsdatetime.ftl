		...
		<#list 1..noOfPages as pageNo>
			...
			<#if eventPosts?has_content>
				<#list eventPosts as post>
					<#attempt>
						<#assign msgStartDatetime = restadmin("/messages/id/${post.id}/metadata/key/microsoft.msg_start_datetime").value />
						<#if msgStartDatetime?length gt 0>
							...
							<#assign setStartDate = restadmin("/messages/id/${post.id}/metadata/key/microsoft.msg_start_day/set?value=${msgStartDatetime?datetime['${customDateFormat}']?long?c}").metadata_association />
							${text.format("general.Time")}: ${restadmin("/messages/id/${post.id}/metadata/key/microsoft.msg_start_day").value}
						</#if>
						<#assign msgEndDatetime = restadmin("/messages/id/${post.id}/metadata/key/microsoft.msg_end_datetime").value />
						<#if msgEndDatetime?length gt 0>
							${text.format("message.id.title")}: ${post.id} 
							${text.format("chat.calendar.end_date.title")}: ${msgEndDatetime}
							<#assign setEndDate = restadmin("/messages/id/${post.id}/metadata/key/microsoft.msg_end_day/set?value=${msgEndDatetime?datetime['${customDateFormat}']?long?c}").metadata_association />
							${text.format("general.Time")}: ${restadmin("/messages/id/${post.id}/metadata/key/microsoft.msg_end_day").value}
						</#if>
					<#recover>
